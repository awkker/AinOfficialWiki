...
> 将文中的 `https://your-relay.example.com` 和 `cr_xxxxxxxxxx` 替换为你的实际地址与密钥。
...
{
  "env": {
    "ANTHROPIC_AUTH_TOKEN": "cr_xxxxxxxxxx",
...
3. 注意：请将 `cr_xxxxxxxxxx` 替换为真实 API Key。环境变量 `ANTHROPIC_AUTH_TOKEN` 与 `ANTHROPIC_BASE_URL` 的优先级高于配置文件。
  </Tab>
...
然后在 `%USERPROFILE%\.codex\auth.json` 写入：
...
💡 将 `OPENAI_API_KEY` 保持为 `null`，并设置环境变量 `CRS_OAI_KEY` 为你的 API 密钥（例如 `cr_xxxxxxxxxx`）。
...
```powershell
# 提醒：将示例值改为你的真实 CRS API Key（格式如 cr_xxxxxxxxxx）
...
$env:GOOGLE_CLOUD_ACCESS_TOKEN = "cr_xxxxxxxxxx"
...
export GOOGLE_CLOUD_ACCESS_TOKEN="cr_xxxxxxxxxx"
